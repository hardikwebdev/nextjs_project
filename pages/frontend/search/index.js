...
import { getAllSavedData, setSavedBlogsData } from "@/shared/services/Front_Apis/account/accountCrud.js";
...
function searchPage() {
    ...
    const [categories, setCategories] = useState([{ value: '', label: 'All' }]); // Add an "All" option to the categories state
    ...
    useEffect(() => {
        getCategories()
            .then(({ data }) => {
                ...
                setCategories([{ value: '', label: 'All' }, ...categoryOptions]); // Include the "All" option at the beginning of the list
            })
            .catch((error) => {
                console.log(error);
            })
    ...
    const getBlogItem = () => {
        ...
        if (accessToken) {
            getAllSavedData(accessToken, page, pageSize, sortBy, sortOrder, false)
                .then(({ data }) => {
    ...
    useEffect(() => {
        const storedSearch = localStorage.getItem('homePageSearch');
        const storedSelectedTags = localStorage.getItem('homePageSelectedTags');
        const storedSelectedCategory = localStorage.getItem('homePageSelectedCategory');
    ...
    useEffect(() => {
        localStorage.setItem('homePageSearch', search);
        localStorage.setItem('homePageSelectedTags', JSON.stringify(selectedTags));
        localStorage.setItem('homePageSelectedCategory', JSON.stringify(selectedCategorie));
    }, [search, selectedTags, selectedCategorie]);
                                                        ...
                                                        <div className="px-5 pb-5">
                                                            ...
                                                            {searchValue ? (
                                                                <>
                                                                    ...
                                                                    <div className="text-center  mt-3">Reset search values to get all Data </div>
                                                                ...
                                                                (
                                                                    <>
                                                                        <div className="text-center empty-state__help">Don't have any data matching with your tag</div>
                                                                        <div className="text-center  mt-3">Reset tag values to get all Data </div>
                                                                        <div className='mt-3 text-center'>
                                                                            <Button
                                                                                variant="danger"
                                                                                className="btn tx-16 px-4"
                                                                ...
                                                                : selectedCategorie && blogs.length === 0 ?
                                                                    (
                                                                        <>
                                                                            <div className="text-center empty-state__help">Don't have any Data matching with your category</div>
                                                                            <div className="text-center  mt-3">Reset category values to get all Data </div>
                                                                            <div className='mt-3 text-center'>
                                                                                <Button
                                                                                    variant="danger"
                                                                                    className="btn tx-16 px-4"
                                                                    ...
                                                                    :
                                                                    <>
                                                                        <div className="text-center empty-state__help">Don't have any Data</div>
                                                                    </>
                                                            }
