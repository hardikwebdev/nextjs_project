...
const Blog = () => {
  ...
  const [totalBlogs, setTotlaBlogs] = useState(null)
  ...
  const handleModal = (data, blog) => {
    setSendStatus(blog?.status);
    setEdit(blog);
    setModalValue(data);
    setAuthModal(true);
  };
    ...
    <div className="pb-5">
      ...
      <>
          ...
          <Card.Body className="py-0">
            <div className="breadcrumb-header justify-content-between">
              <div className="left-content">
                <span className="main-content-title mg-b-0 mg-b-lg-1">Blogs</span>
              </div>
            </div>
          </Card.Body>
        </Card>
                ...
                <Row className="align-items-center">
                  ...
                  <Col md={6}>
                    <div className="main-content-label mb-0 ">
                      <Row>
                        <Col xs={12} md={9} className="mt-sm-2 mt-0">
                        ...
                        <Col xs={12} md={2} className="mt-sm-2 mt-0">
                          <div>
                      ...
                      <div className="ms-4">
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="tx-15">Add blog</span>
                          </Button>
                        </Link>
                      </div>
                      {/* Add category end */}
            ...
            blogs.map((blog) => (
              ...
              >
                <Card className="card custom-card card-img-top-1 h-100">
                  <div>
                    {blog?.parsedBanner.length > 0 ?
                      <>
                        {blog?.parsedBanner[0].type === "blog_image" ? (
                          <img
                            className="blog-img w-100 position-relative"
                            src={blog?.parsedBanner[0]?.Location}
                            alt={blog?.title}
                          />
                        ) : (
                          null
                        ...
                        <>
                          {blog?.parsedBanner[0].type === "blog_video" ? (
                            <VideoComponent videoSrc={blog?.parsedBanner[0]?.video_thumbnail?.Location} videoDuration={blog?.parsedBanner[0]?.video_duration} styling={`blog-img`} />
                          ) : (
                            null
                          )}
                        </>
                    ...
                    <div className="Active-badge position-absolute">
                      <span
                        className={`text-${blog?.status === 1 ? "success" : "danger"
                          } cursor-pointer tx-13`}
                        onClick={() => {
                          handleModal(
                            blog?.status === 1 ? "Inactive" : "Active",
                            blog
                          );
                        }}
                      >
                        <i
                          className={`fe ${blog?.status === 1
                            ? "fe-check-circle"
                            : "fe-alert-octagon"
                            } tx-13 me-1`}
                        ></i>
                        <span>{blog?.status === 1 ? "Active" : "Inactive"}</span>
                      </span>
                    </div>
                  </div>
                  <Card.Body className="pb-0 d-flex flex-column ">
                    <div className="d-flex">
                      {blog?.content_type === "paid" && ( // Check content_type and conditionally render the premium badge
                      ...
                      <div className={`category-badge ${blog?.content_type === "paid" ? 'ms-2' : null}`}>
                    ...
                    <div className="text-muted d-flex align-items-center mt-3">
                      <span className="m-0">{blog?.user?.first_name}</span>
                    ...
                    <div className="mt-2">
                      <h4 className="card-title text-dark m-0 tx-12">{blog?.title}</h4>
                    </div>
                    <div className="text-muted mt-2">
                      <span className="m-0 tx-11">{blog?.short_description}</span>
                    </div>
                    <div className="mg-t-auto">
                      <div className="horizontal-row"></div>
                      <div className="d-flex align-items-center">
                        <div>
                          {/* <span className={`text-info tx-16 cursor-pointer `} onClick={() => handlePandingComents(blog?.id)}>
                            Comments: {blog?.commentsCount}
                          </span> */}
                          <span
                            className={`text-info tx-16  ${blog?.commentsCount === 0 ? '' : 'cursor-pointer'}`}
                            onClick={() => blog?.commentsCount !== 0 && handlePandingComents(blog?.id)}
                          >
                            Comments: {blog?.commentsCount}
                          </span>
                        ...
                        <div className="my-2 ms-auto">
                          ...
                          >
                            ...
                            <Link href={`/admin/blogs/edit-blog`}
                              className="fe fe-edit"
                              onClick={() => handleEditData(blog)}
                            >
                            </Link>
                          </OverlayTrigger>
                          <OverlayTrigger
                          ...
                          >
                            <i
                              className="fe fe-trash-2 ms-2 text-dark cursor-pointer"
                              onClick={() => {
                                DeleteAlert(blog?.id);
                              }}
                            ></i>
                ...
                <Card>
                  <div className="mt-2">
                    <div className="px-5 pb-5">
                      ...
                      {isFilterActive || searchValue ? (
                        <>
                          <div className="text-center empty-state__help">Don't have any Blog matching with your filter</div>
                          <div className="text-center  mt-3">Click "Reset" button to get all Blogs or try another filter</div>
                        </>
                      ) : (
                        <>
                          <div className="text-center empty-state__help">You don't have any Blog</div>
                          <div className="text-center  mt-3">Click "Add blog" to create new blog</div>
                        </>
                      )}
                    </div>
                  </div>
        ...
        <Modal show={authModal} onHide={() => setAuthModal(false)}>
          <Modal.Header>
            <Modal.Title className="text-dark">Update status</Modal.Title>
            <Button
              variant=""
