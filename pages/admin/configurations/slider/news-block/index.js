...
export default function News() {
  ...
  const getSliderData = () => {
    ...
    getSliders(accessToken, "news", page, pageSize, sortBy, sortOrder)
      .then(({ data }) => {
                  ...
                  <tbody>
                    ...
                    {data?.map((data, index) => (
                      <tr key={data?.id}>
                        ...
                        <td className={`text-${data?.status === 1 ? "success" : "danger"}`}>
                          <div className="Active-badge-user">
                            <span
                              className={`user-${data?.status === 1 ? "active" : "Inactive"} cursor-pointer tx-13`}
                              onClick={() => {
                                handleModal(
                            ...
                            >
                              <i className={`fe fe-${data?.status === 1 ? "check-circle" : "alert-octagon"} tx-12 me-2`}></i>
                              <span className="tx-12">{data?.status === 1 ? "Active" : "Inactive"}</span>
                            </span>
                          </div>
                        </td>
                        <td>
                          <div className="d-flex justify-content-center align-items-center">
                            ...
                            >
                              <Link
                                href={`/admin/configurations/slider/news-block/Edit-block`}
