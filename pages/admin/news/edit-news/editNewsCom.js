...
const EditNewscom = () => {
  ...
  useEffect(() => {
    const TagOptions = blogData?.blogstags?.map((Tag) => Tag?.tags?.name);
    setSelectedTag(TagOptions);
    if (blogData && blogData.publish_date) {
      const publishDate = new Date(blogData.publish_date);
    ...
    if (blogData) {
      setDataForPreview(blogData)
    }
    if (blogData && blogData.content_type) {
      const content_Type = blogData.content_type;
      setContentType(content_Type);
    }
    if (blogData && blogData.long_description) {
      const long_description = blogData.long_description;
      setLongDiscContent(long_description);
    }

    setBannerType(blogData?.parsedBanner[0]?.type === "blog_video" ? "Video" : "Image");

    setImagePreview(blogData?.parsedBanner)
    setbase64ImageArr(blogData?.banner)
    setParsedBanner(JSON.parse(blogData?.banner));
  ...
  useEffect(() => {
    if (blogData && blogData?.category_id) {
      const selectedCategoryMatch = categories.find(
        (category) => category.value === blogData?.category_id
      );
      if (selectedCategoryMatch) {
        setSelectedCategory(selectedCategoryMatch);
      }
    }
  }, [categories]);
  ...
  useEffect(() => {
    if (blogData && blogData.category_id) {
      const selectedAuthorMatch = authors.find(
        (author) => author.value === blogData.user_id
      );
      if (selectedAuthorMatch) {
        setSelectedAuthor(selectedAuthorMatch);
      }
    }
  }, [authors]);
